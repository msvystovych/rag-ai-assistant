...
|---|---|---|---|---|
| 1 | **Residual short chunks.** Some chunk bodies fall under 500 chars, most often a section's trailing piece, a whole short section, or a document's opening piece. | `merge_short` merges any undersized piece backward. The validation report counts what survives. Simulation says single-digit percent, and zero bodies under 250 chars. Report the real number honestly rather than padding text to hit it. See § Settled decisions **D1** in [`pipeline-spec.md`](pipeline-spec.md). | **13 of 77 (16.9%)** — nearly double the simulated 8.5%, and barely better than the 14.6% no-merge baseline. The rule fired **once**: it saw 14 candidates, it merged 1, and the ≤1000 cap refused the other 13. Zero bodies under 250 chars, as predicted. | **Mitigation ineffective at chunk_size 800.** A ~800-char predecessor plus a ~79-char breadcrumb leaves ~110 chars of headroom, so almost no merge fits. The 500/100 variant merged 17 of 17 — the rule works, but the parameters starve it. |
| 2 | **Breadcrumb overhead.** The prefix consumes 40–90 chars of every chunk's budget. It repeats across a section's chunks. | The design accepts this cost for standalone readability. Improvement: shorten long H1 titles, or drop the title from the breadcrumb when the section name already disambiguates. | Mean overhead **79 chars/chunk** (~11% of mean chunk length), inside the predicted 40–90 band. | **Trade accepted, with a caveat found in HW2.** Readability held. But the pipeline prepends the H1 to every chunk. So a query that repeats title wording gets a document-wide lexical boost (see `q08`, `docs/homework2/analysis.md`). |
| 3 | **Overlap quality.** A naive 150-char tail can start mid-sentence. | Improvement: snap the overlap window back to the previous sentence boundary so repeated context reads cleanly. | **Confirmed.** The carry snaps to a *word* boundary only. So chunks routinely open mid-sentence ("time and handling risk. That distinction…"). The top-1 hits for `q01` and `q09` show this. | **Still open.** Ranking stays unaffected (the embedding sees the whole chunk); readability does not. Sentence-boundary snapping remains the right fix. |
| 4 | **List- and table-heavy sections chunk worse than prose.** Vocabulary and checklist sections fragment badly and embed poorly. | Author those sections as full sentences ("A lane is…"). [`corpus-plan.md`](corpus-plan.md) holds the guidance. | **Avoided by construction** — the authored corpus contains no tables and no multi-item lists. So the failure mode never materialised. | **Mitigation worked.** The Key Domain Vocabulary section, the most list-prone by subject, chunked normally as prose. |
...
| 6 | **Sanitization drift.** A platform-specific detail can slip in during writing. | The authoring voice rule plus the pre-submission grep pass in [`corpus-plan.md`](corpus-plan.md). The knowledge base contains no personal data by construction. |
...
The script's summary output should give you all of these. They turn this file from
