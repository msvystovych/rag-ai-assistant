...
top of it. That means the prompt versions, the relevance floor, the citation contract, and the
grounded-QA evaluation. Homework #2 closed with "No LLM answer generation. This homework ends at
retrieved chunks." — this homework consumes that deferral.
...
- **The floor takes its calibration from one statistic and its input from another.** 0.35 came from
  ...
  the chunks the **RRF-fused top-k** returned. Under hybrid retrieval those are not the same
  number. The arithmetic is unforgiving. A chunk that both branches find scores at least
  `1/(60+10) + 1/(60+10) = 0.0286`. A chunk that the semantic branch ranked **first**, and that
  BM25 never surfaced, scores `1/(60+1) = 0.0164`. Three dual-branch chunks therefore displace the
  semantic rank-1 chunk out of a `k=3` result. The gate then judges the question on cosine values
  from lower down the semantic ranking. Homework #2 derived 0.35 from higher-ranked values than
  those. The direction of the error is toward **false refusal** — the safe direction, but a failure
  all the same. **The committed output measures this effect; it is not a hypothesis.**
  `outputs/rag_answers_results.json` records `semantic_rank` for every retrieved chunk. A record
  whose chunks include no `semantic_rank: 1` is one where the gate judged a displaced statistic.
  Exactly one of the ten does: **q05**. Its returned chunks are semantic ranks 5, 4 and 2, and the
  semantic rank-1 chunk never reached the context. Query **q05** also has the narrowest margin in
  the set (+0.062 over the floor). The one query with a displaced statistic is therefore the one
  closest to refusal. The two effects compound on the same query rather than cancelling.
  Recalibrating properly would mean measuring the fused-top-k cosine distribution, which needs a
  run this homework does not budget for.
- **The floor's margin is thin.** 0.35 sits only 0.063 below the lowest measured in-corpus top-1
  (0.413). The floor would refuse a genuinely answerable question whose best chunk lands just under
  it. The failure mode is a false refusal, not a hallucination. That direction is the safer one,
  but it is still a failure. `outputs/rag_answers_results.json` records `top_semantic_score` for
  every question precisely so a near-miss stays visible rather than silent. `--no-min-score`
  disables the gate entirely.
- **The floor reads the semantic score only.** A chunk that only BM25 surfaced carries no
  `semantic_score`. The floor treats a set of nothing but such chunks as "no semantic evidence" →
  empty context. That is the honest reading of the available signal, but it means the floor
...
- **The citation parser recognises a bounded set of shapes.** It reads bracketed spans. It accepts
  ...
  citations for a model that cited in prose ("according to chunk a_chunk_001"). The undercount
  is conservative for the citation rate and **unsafe** for the fabricated-citation count. The
  parser therefore matches the schema shape rather than trusting the brackets.
- **Citation counting measures attribution, not correctness.** A `[chunk_id]` marker proves that
...
- **The weak-context half of the fallback is measured, but only outside the committed run.** The
  ...
  to use, and no in-corpus question ended in a refusal. The weak half therefore needed a separate
  run with the floor disabled. The block below keeps the four lines that carry the result;
  `format_answer` also prints a `Question:`, a `Prompt:` and a `Source:` line:
  ...
  Three genuinely off-topic chunks reached the model, and it declined to use them. The prompt
  rule therefore refuses a **non-empty** insufficient context on its own. The two gates are
  independently effective, not merely independently described. This run is not part of the
  committed evaluation, because it would change the graded aggregates. This document records the
  run for that reason.

- **That run also exposed a defect, now fixed.** With the floor disabled the refusal was correct,
  but the `Source:` line named both retrieved documents. `source_files()` falls back to every
  retrieved chunk when the model cites nothing. That fallback is reasonable for an uncited
  *answer*, and exactly wrong for a *refusal*. With the floor on, every refusal also had an empty
  context, so the earlier gate hid the case. Refusals now report no source when the floor withheld
  the context, and when the model merely declined it. Tests pin both branches. The committed
  artifacts never carried the defect, because q10's context was empty there. The evidence run for
  the bullet above did carry it.
...
- **No LLM-as-judge answer scoring.** It would produce impressive-looking numbers whose validity
  rests on the judge. This homework instead keeps its evidence readable by a human in
  `outputs/rag_answers_examples.md`.
- **No answer caching.** `--evaluate` runs 10 questions and makes 10 chat calls. `--improvements`
  runs 3 questions and makes 6 chat calls, because each of its 3 cases runs two prompt versions
  over one retrieval. Both modes run at temperature 0. A cache would add an invalidation problem
  for no measurable gain.
- **No agentic re-query loop** — no "the context was thin, so retrieve again with different terms".
  That is a different system. The rubric grades the honest refusal instead.
...
- **No structured/JSON output mode.** The rubric grades a readable grounded answer with a citation.
  Forcing a schema would trade that graded thing for machine-parseability nothing consumes.
