...
def evaluate(settings: Settings, queries: list[dict[str, Any]], k: int) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "top1_document": hits[0].metadata.get("document_id", "") if hits else "",
                "top1_section": hits[0].metadata.get("section", "") if hits else "",
                "documents_in_topk": len({hit.metadata.get("document_id", "") for hit in hits}),
                "sections_in_topk": len({hit.metadata.get("section", "") for hit in hits}),
                "verdict": verdict(hits, expected),
                "hit": expected and hits and hits[0].metadata.get("document_id", "") in expected,
            }
        )
    return rows
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, float]:
    ...
    return {
        ...
        "mean_documents": statistics.mean(row["documents_in_topk"] for row in in_corpus),
        "out_of_corpus_top1": out_of_corpus[0]["top1"] if out_of_corpus else float("nan"),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--queries", type=Path, default=REPO_ROOT / "data/eval/test_queries.json")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--baseline-index", type=Path, default=REPO_ROOT / "index/chroma")
    parser.add_argument("--baseline-collection", type=str, default="logistics_chunks")
    parser.add_argument("--baseline-label", type=str, default="chunk_size 800 / overlap 150")
    parser.add_argument("--variant-index", type=Path, default=REPO_ROOT / "index/chroma_500")
    parser.add_argument("--variant-collection", type=str, default="logistics_chunks_500")
    parser.add_argument("--variant-label", type=str, default="chunk_size 500 / overlap 100")
    parser.add_argument(
        "--baseline-chunks", type=Path, default=REPO_ROOT / "data/processed/chunks.jsonl"
    )
    parser.add_argument(
        "--variant-chunks", type=Path, default=REPO_ROOT / "data/processed/chunks_500.jsonl"
    )
    ...
    report = render(
        ...
        baseline_chunks=count_lines(args.baseline_chunks),
        variant_chunks=count_lines(args.variant_chunks),
    )
