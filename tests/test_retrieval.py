...
from run_test_queries import verdict  # noqa: E402
...
class TestSettings:
    def test_missing_api_key_is_a_diagnostic_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestEmbedding:
    def test_preserves_input_order_across_batches(self, settings: Settings) -> None:
        ...
        assert len(client.calls) == 2, "should batch rather than issue one call per text"
...
class TestManifest:
    ...
    def test_model_mismatch_is_refused(self, index: Settings) -> None:
        mismatched = Settings(**{**index.__dict__, "embedding_model": "text-embedding-3-large"})
...
class TestSearch:
    ...
    def test_query_is_embedded_with_the_same_model_as_the_chunks(self, index: Settings) -> None:
        client = FakeOpenAI()
